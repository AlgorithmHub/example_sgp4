...
% -------------------------  implementation   -------------------------
        ...
        while ((abs(xn-x) >= small) && (loops <= 30))
            x    = xn;
            tempx  = seebatt(x);
%             % alt approach not using continued fractions from Wenzel, not
%             % working yet
%             eta = x/(sqrt(1.0+x)+1.0)^2; 
%             if abs(eta) < 0.0000001
%                  tempx = 5.0;
%              else 
%                  if eta < 0.0
%                      xx = sqrt(-eta);
%                      ax = atanh(sqrt(abs(xx)));
%                      tempx = 4.0*xx*xx*(ax - xx) / ((3.0-xx*xx)*ax - 3.0*xx);
%                  else
%                      xx = sqrt(eta);
%                      ax = atan(abs(sqrt(xx)));
%                      tempx = 4.0*xx*xx*(xx - ax) / ((3.0+xx*xx)*ax - 3.0*xx);
%                  end
%              end 
