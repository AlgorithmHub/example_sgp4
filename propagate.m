function [r,v] = propagate(longstr1, longstr2, t)
...
%     // ------ correct fix will occur when year is 4-digit in 2le ------
    ...
    [satrec] = sgp4init(wgs, satrec, satrec.bstar, satrec.ecco, sgp4epoch, satrec.argpo, satrec.inclo, satrec.mo, satrec.no, satrec.nodeo);
    ...
    v = zeros(3,len);
    for ii = 1:len
        [satrec,r(:,ii),v(:,ii)] = sgp4(satrec,t(ii));
        [ut1, tut1, jdut1, utc, tai, tt, ttt, jdtt, tdb, ttdb, jdtdb ] = convtime ( year, mon, day, hr, minute, sec, timezone, dut1, dat );
        [r(:,ii),v(:,ii)] = teme2ecef(r(:,ii),v(:,ii),[0;0;0],ttt,jdut1,lod,xp,yp);
    end
