...
function [atwa, atwb, atw, b, drng2, daz2, del2 ] = findatwaatwb(firstobs, lastobs, obsrecarr,  statesize, percentchg, deltaamtchg, xnom);
    ...
    for obsktr = firstobs: lastobs
        ...
        for j= 1: statesize
            ...
            dtsec = (obsrecarr(obsktr).time - obsrecarr(1).time) * 86400;
            rnomp(1) = xnomp(1,1);
            rnomp(2) = xnomp(2,1);
            rnomp(3) = xnomp(3,1);
            vnomp(1) = xnomp(4,1);
            ...
            if (currobsrec.obstype == 3)
                [trr,trtasc,tdecl,tdrr,tdrtasc,tddecl] = rv2tradc( reci3', veci3',latgd,lon,alt,ttt,jdut1,lod,xp,yp,terms,ddpsi,ddeps );
            else
                if (currobsrec.obstype == 4)
